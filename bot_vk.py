...
def create_new_keyboard1(info_for_keyboard):
    begin, finish, index, name, text_for_buttons = info_for_keyboard
    keyboard = VkKeyboard(**settings_keyboard_inline)

    if text_for_buttons != "cart":
        for num in range(begin[index], finish[index]):
            keyboard.add_button(label=f"{name[num]}", color=VkKeyboardColor.SECONDARY, payload={"type": f"{text_for_buttons}"})
            keyboard.add_line()
    else:
        for num in range(begin[index], finish[index]):
            keyboard.add_callback_button(label=f"{name[num][0]} - {name[num][1]}", color=VkKeyboardColor.SECONDARY,
...
def keyboard_good_info(id):
    keyboard = VkKeyboard(**settings_keyboard_inline)
    if users_id_info[id][12] > 1:
        keyboard.add_callback_button(label='-1', color=VkKeyboardColor.NEGATIVE, payload={"count": "-1",
                                                                                                       "count_good": "change"})
    keyboard.add_callback_button(label=f"Количество - {users_id_info[id][12]}", color=VkKeyboardColor.PRIMARY, payload={"count": " "})
    keyboard.add_callback_button(label='+1', color=VkKeyboardColor.POSITIVE, payload={"count": "+1", "count_good": "change"})
    keyboard.add_line()
        ...
        if event.obj.message["text"] != '':
            if event.from_user:
                if event.obj.message['from_id'] not in users_id_info:
                    '''0- информация о товаре(карточка), 1- общая информация для вывода (карточка)
                       11- корзина, 12- добавить количество товара в корзину и сам товар, 13 - длина корзины, 
                       14- информация о сформированных заказах,
                       ...
                       0 - шаг регистрации + информация для регистрации: [шаг регистрации, id пользователя, имя, номер телефона, дата рождения]
                       1- данные для клавиатуры категорий и товаров, 2- корзина, 3- карточка товара
                    '''
                ...
                # '''Вывод первичного меню категорий товаров'''
                    ...
                    if event.obj.message["text"].lower() in start_bot_msg:
                        ...
                        vk.messages.send(
                            ...
                            random_id=get_random_id(),
                            peer_id=event.obj.message['from_id'],
                            keyboard=create_new_keyboard1(users_id_info[event.obj.message['from_id']][1]).get_keyboard(),
                ...
                if event.obj.message["text"] in categories:
                    ...
                    users_id_info[event.obj.message['from_id']][1] = [[i for i in range(0, len_goods, 4)],
                                                                      ...
                                                                      0, [name[0] for name in category_goods[event.obj.message["text"]]], "goods"]
                    vk.messages.send(
                        user_id=event.obj.message['from_id'],
                        random_id=get_random_id(),
                        peer_id=event.obj.message['from_id'],
                    ...
                    users_id_info[event.obj.message['from_id']][3] = params[0] + good_card_info[2] + "\n" + \
                    ...
                    vk.messages.send(
                        ...
                        message=users_id_info[event.obj.message['from_id']][3])
                # """"""
                ...
                if users_id_info[event.obj.message['from_id']][21] == "add address":
                    ...
                    correct_delivery_address(users_id_info[event.obj.message['from_id']][20],
                    ...
                    users_id_info[event.obj.message['from_id']][10].add_callback_button(
                        label='Продолжить без комментария',
                        color=VkKeyboardColor.POSITIVE,
        ...
        if event.object.payload.get('type') in CALLBACK_TYPES:
            ...
            if event.object.payload.get("reg_user") == "yes":
                ...
                vk.messages.send(
                    ...
                    random_id=get_random_id(),
                    peer_id=event.object.peer_id,
                    keyboard=users_id_info[event.object.user_id][1].get_keyboard(),
                    message="Выберите доступную категорию товаров:")
            ...
            if event.object.payload.get('count_good') == "change":
                ...
                vk.messages.edit(
                    peer_id=event.obj.peer_id,
                    message=users_id_info[event.object.user_id][3],
            ...
            if event.object.payload.get("type") == "basket":
                ...
                if not users_id_info[event.object.user_id][11]:
                    ...
                    users_id_info[event.object.user_id][5] = [i for i in
                                                              range(0, users_id_info[event.object.user_id][13], 4)]
                    users_id_info[event.object.user_id][6] = [i for i in
                                                              range(4, users_id_info[event.object.user_id][13], 4)] + \
